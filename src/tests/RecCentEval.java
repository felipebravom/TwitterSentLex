...
public class RecCentEval {

	private String path; // input Path
	private String lexPath; // Path of destination Lex
	private WordDocumentRecCent recCentFilt;  // the TextToVSM Filter to be used 
	private int iterations; // defines the type of Filter to be used

	public RecCentEval(String path, int iterations, String lexPath){
		this.path=path;	
	...
	public void evalWordLevel() throws Exception{
		...
		System.out.println(" ,WordDocumentRecCent");



		this.recCentFilt=new WordDocumentRecCent();
		this.recCentFilt.setIterations(iterations);
		
		System.out.println("Iterations,"+iterations);



		this.recCentFilt.setOptions(Utils.splitOptions("-M 10 -N 10 -C -I 3 -P WORD- -Q CLUST- -L -K -J lexicons/metaLexEmo.csv -H resources/50mpaths2.txt -R -T resources/stopwords.txt -O -S"));
		this.recCentFilt.setInputFormat(train);
		Instances wordVSM=Filter.useFilter(train, this.recCentFilt);
		...
		saver.setInstances(wordVSM);
		saver.setFile(new File(this.lexPath+".arff"));
		saver.writeBatch();
	...
	static public void main(String args[]) throws Exception {
		...
		System.out.println("---------------------------------");
		RecCentEval vsmEv=new RecCentEval(args[0],Integer.parseInt(args[1]),args[2]);
		vsmEv.evalWordLevel();
