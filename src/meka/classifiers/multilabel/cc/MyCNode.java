 ...
 * @version June 2013
 */
public class MyCNode implements Serializable {
	 ...
	 */
	public MyCNode(int j, int inX[], int paY[]) {
		this.j = j;
		this.inX = inX;
		this.paY = paY; 
	}
	...
	*/
	...
	public double classify(Instance x, double ypred[]) throws Exception {
		Instance x_ = transform(x,ypred);
		double disForInst[]=h.distributionForInstance(x_);
		return disForInst[1];
	}
	...
	public static void main(String args[]) throws Exception {
		...
		for(int j : s) {
			...
			MyCNode n = new MyCNode(j,null,pa_j);
			n.build(D,new SMO());
			/*
			 */
